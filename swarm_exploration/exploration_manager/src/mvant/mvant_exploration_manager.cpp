...
int MvantExplorationManager::planExploreMotion(const Vector3d& pos, const Vector3d& vel, const Vector3d& acc, const Vector3d& yaw) {
  ...
  //findPathClosestFrontier(pos, vel, yaw, next_pos, next_yaw);
  //explorerPlan(pos, vel, yaw, next_pos, next_yaw); 
  success = multiVantFrontier(pos, yaw, next_pos, next_yaw);
...
bool MvantExplorationManager::findPathClosestFrontier(const Vector3d& pos, const Vector3d& vel,
  ...
  double compute_direction_cost(const Eigen::Vector3d& robot_position,const Eigen::Vector3d& robot_vel, const Eigen::Vector3d& frontier_position){
    ...
    //double dot = robot_vel.head(2).normalized().dot(direction);
    //double direction_cost = ViewNode::w_dir_ * (1.0 - (robot_vel.head(2).normalized().dot(direction.normalized())));
  ...
  double MvantExplorationManager::compute_comm_penalty(int robot, const Eigen::Vector3d& frontier_position) const{
    // Si solo hay un robot, no aplica la penalizacion
    const int drone_num = ed_->swarm_state_.size()-1;
    if(drone_num <= 1){
      return 0.0;
    }

    double rango_comm = 30.0; // metros?
    const auto& dr = ed_->swarm_state_[robot];
    ...
    for(int j = 0; j < drone_num; ++j){
      ...
      if(j==robot) continue;
      const auto& dr_j = ed_->swarm_state_[j];
      
      Eigen::Vector3d center = dr_j.goal_pos_;

      double d_start = (dr.pos_ - center).norm();
      double d_end   = (frontier_position - center).norm();

      // Si ambos extremos de la trayectoria estan dentro del mismo circulo
      // asumimos que el camino completo queda cubierto por ese nodo
      if(d_start <= rango_comm && d_end <= rango_comm){
        bridge_ok = true;
        break;
      }
    }
  ...
  bool MvantExplorationManager::multiVantFrontier(const Vector3d& pos, const Vector3d& yaw, Vector3d& next_pos, double& next_yaw, bool force_different)  {
    ...
    // Histeresis: penaliza cambiar de objetivo (reduce "brincos")
    const double GOAL_MATCH_TOL  = 0.8;   // [m] si goal_pos_ está "cerca" del viewpoint, lo consideramos mismo objetivo
    const double SWITCH_PENALTY  = 200.0;  // penalización base por cambiar (suba si brinca mucho)

    // Radio de bloqueo: si una frontera está muy cerca del objetivo de otro robot
    const double R_BLOCK         = 4.0;     // [m] radio de bloqueo alrededor del goal de otros robots
    const double BLOCK_PENALTY   = 5e4;     // grande (pero < mat.setConstant(...) si quiere que siga siendo elegible)

    // Helper: ¿esta frontera corresponde al goal actual?
    auto same_goal = [&](const Eigen::Vector3d& goal, const Eigen::Vector3d& cand) -> bool {
      return (goal - cand).norm() < GOAL_MATCH_TOL;
    };
    ...
    //ROS_WARN_STREAM("robots_ " << drone_num);
    //ROS_WARN_STREAM("fronteras_ " << ftr_num);
    ...
    for (int r = 0; r < drone_num; ++r) {
      ...
      ROS_WARN_STREAM("objetivo drone = " << ed_->next_pos_.transpose());
      
      //ITERAR EN FRONTERAS de lista armada arriba 
      for (const auto& ftr : fronteras_list) {
        //*****************************************
        ...
        //PARTE MULTI ROBOT
        for (int j = 0; j < drone_num; ++j) {
          if (j == r) continue;  // todos menos yo
          ...
          //Si las fronteras están lejos de los otros robots,
          //rho_j + alpha_ji es grande sum grande.
          sum += rho_j + alpha_ji; 

          //Ahora es grande cuando la frontera esta cerca de 
          //otros robots (o de sus metas)
          //sum += 1.0 / (rho_j + alpha_ji + eps);

        }

        if(drone_num > 1){
          exploracion = sum / (drone_num - 1);
        }

        //double comm_penalty = compute_comm_penalty(r, vj.pos_);  // 0 si el puente se mantiene
        double cost = explotacion - exploracion;// + hyst_cost + block_cost;
        ROS_WARN_STREAM("explotacion = " << explotacion);
        ROS_WARN_STREAM("exploracion = " << exploracion);
        ROS_WARN_STREAM("cost = " << cost);
        //function costo va aqui
        // r - robot; index - frontera
        mat(r,_ftr_) = cost; 
        //mat(r,_ftr_) = explotacion - exploracion; //+ comm_penalty;
        ++_ftr_;
      }
